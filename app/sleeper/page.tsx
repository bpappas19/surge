...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    in_season: { label: "Live",      cls: "text-emerald-400 bg-emerald-500/10 border-emerald-500/25" },
  ...
  return (
    <span className={`text-[10px] font-semibold tracking-wide px-1.5 py-0.5 rounded border ${s.cls}`}>
      {s.label}
    </span>
  );
}
...
function LeagueRow({ league, onClick }: { league: SleeperLeague; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="group w-full flex items-center gap-3 px-4 py-3.5 hover:bg-white/3 active:bg-white/5 transition-colors text-left"
    >
      {img ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={img} alt={league.name} className="w-9 h-9 rounded-lg object-cover flex-shrink-0" />
      ) : (
        <div className="w-9 h-9 rounded-lg bg-white/5 border border-white/6 flex items-center justify-center flex-shrink-0">
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[520px] sm:max-w-[620px] lg:max-w-[720px] bg-[#0d1420] border border-white/6 rounded-2xl overflow-hidden">
        ...
        {/* Card header */}
        <div className="flex items-start gap-3 px-8 pt-7 pb-5 lg:px-10 lg:pt-9 lg:pb-7 border-b border-white/6">
          <Link href="/" className="text-slate-500 hover:text-slate-300 transition-colors p-1 -ml-1 flex-shrink-0 mt-0.5">
            <ChevronLeft className="w-5 h-5" strokeWidth={1.5} />
          </Link>
          <div>
            <p className="text-sm lg:text-base font-semibold text-white">Connect Sleeper</p>
        ...
        {leagues !== null && (
          leagues.length === 0 ? (
            <div className="border-t border-white/6 px-5 py-8 text-center">
        ...
        {/* Different season toggle */}
        <div className="border-t border-white/6 px-5 py-3">
          <button
            type="button"
            onClick={() => setShowYearSelector((v) => !v)}
